...
def get_init_embedding(reversed_dict, embedding_size):
    glove_file = "/home/pnagula/glove.42B.300d.txt"
    word2vec_file = get_tmpfile("word2vec_format.vec")
    glove2word2vec(glove_file, word2vec_file)
    tf.logging.info('Loading Glove vectors...')
    ...
    tf.logging.info('#########################################Percent of embeddings found:'+str(no_of_embed/(len(reversed_dict)*1.0)*100.0)+'% #########################################')
    # Assign random vector to [UNK], [PAD], [START] and [STOP] token
    word_vec_list[0] = np.random.normal(0, 1, embedding_size)
    word_vec_list[1] = np.random.normal(0, 1, embedding_size)
